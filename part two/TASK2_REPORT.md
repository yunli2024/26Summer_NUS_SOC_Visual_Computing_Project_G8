...
5. Load `artifacts/expression_classifier.joblib` and predict seven-class probabilities.
...
The classifier-only time is displayed separately from total FPS. The offline repeated single-image benchmark was 23.41 ms, below the required 30 ms threshold.
...
- The model uses geometry only; skin texture and wrinkles are unavailable.
