...
- `beginner/`：
  - `starter.py`：最小 webcam display 示例。
  - `haarcascade_frontalface_default.xml`：OpenCV Haar face detector。
  - `lbfmodel.yaml`：OpenCV LBF 68-point facial landmark 模型。
- `expert/`：
  - `facial_expression_dataset.zip`：FER-style facial expression dataset。zip 内含完整 `train`/`test` 七类数据：`angry`、`disgust`、`fear`、`happy`、`neutral`、`sad`、`surprise`。
  - `facial_expression_dataset/`：当前解压目录不完整，只看到 `test/happy`。实现前必须验证数据完整性；如需要完整训练/测试数据，应从 zip 正确解压或读取 zip 内容，不要基于当前不完整目录得出错误结论。
- `bonus/`：
  - `danceapp.py`：Tkinter GUI starter，左右两个 panel，左侧 reference video，右侧 webcam，内置 YOLOv8 pose keypoint visualization。
  - `dance_example_1.mp4`：示例 reference video。
...
- 使用 test set 评估并报告：
  ...
  - failure cases。
  - 遇到的挑战和改进策略。
- 可以展示改进前后结果；cross-validation 是可选项。
