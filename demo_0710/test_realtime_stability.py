...
from expression_features import FaceFeatures, build_feature_vector
...
def test_landmark_jitter_is_smoothed_per_track() -> None:
    ...
    assert not np.allclose(tracked[0].face.vector, jittered.vector)
...
def test_yunet_detector_rejects_room_like_background() -> None:
    detector = YuNetFaceDetector(Path(__file__).resolve().parent / "face_detection_yunet_2023mar.onnx")
