"""Zhangyx Part Two facial-landmark geometry features.

This compatibility module keeps the callable names embedded in the retained
scikit-learn Geometry-SVM pipeline.  It intentionally contains no image
features: every value is derived from 68 eye-aligned LBF landmarks.
"""
...
def normalize_landmarks(landmarks: np.ndarray) -> np.ndarray:
    """Remove translation, in-plane rotation, and scale; return 136 values."""
...
def landmark_geometry_features(features: np.ndarray) -> np.ndarray:
    """Derive 38 expression-focused distances, ratios, and asymmetries."""
    ...
    mouth_corner_angle = np.arctan2(
        mouth_corner_vector[:, 1], mouth_corner_vector[:, 0]
    )
...
def append_landmark_geometry(features: np.ndarray) -> np.ndarray:
    """Append all 38 geometry values to the 136 normalized coordinates."""
    rows = np.asarray(features, dtype=np.float32)
    if rows.ndim == 1:
        rows = rows.reshape(1, -1)
...
def append_landmark_geometry_groups(
    ...
    """Append only requested named groups (used by the ablation model)."""
    rows = np.asarray(features, dtype=np.float32)
    if rows.ndim == 1:
        rows = rows.reshape(1, -1)
    ...
    selected_names = [
        name for group in groups for name in GEOMETRY_FEATURE_GROUPS[group]
    ]
